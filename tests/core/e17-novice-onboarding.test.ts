...
describe('novice installed guidance contract', () => {
  it('provides a first-run security gate, guided practice, and pre-install disclosure', () => {
    ...
    expect(modal).toContain('目前 Hans SafeDoc v1.2 可用');
    ...
    expect(readme).toContain('v1.2 支援下列唯讀來源');
    ...
    expect(help).not.toContain('按「匯入處理結果」');
    expect(help).not.toContain('按「建立還原副本」');
